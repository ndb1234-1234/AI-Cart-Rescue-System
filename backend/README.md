...
Instead of building a single decision tree, XGBoost builds many trees sequentially.
...
meaning the final model contains 800 boosted decision trees.
...
(x) = Prediction from individual tree
...
To address this issue, SMOTE was applied.
...
78% probability that the customer will abandon the cart.
Model Evaluation
...
The Area Under the Curve (AUC) measures how well the model distinguishes between abandoned and purchased sessions.
...
However, analysis showed that many purchased and abandoned sessions exhibited similar behavioral characteristics.

This limited the amount of predictive signal available to the model.
